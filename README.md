...
Rather than read about it, I wanted to measure it.
...
**Rail-optimized removed all variance.** Every one of 128 flows finished in exactly 91.0 µs — 1.03× the theoretical floor. On the fat-tree, the median was 178.7 µs but the worst flow took 959.9 µs
...
**Ring ordering mattered as much as the hardware.** Running the same collective on the same rail-optimized fabric, but ordering the ring naively (0 → 1 → 2 → …), made every hop cross-rail and forced traffic up to the spine tier. Estimated collective time went from 2.73 ms to 18.0 ms — 6.6× worse

![Estimated collective time](plots/collective_time.png)

**The advantage only appears with large messages.** Sweeping flow size from 1 KB to 4 MB, the fat-tree's penalty is invisible below ~10 KB and severe above ~1 MB
...
**Rail-optimized wins by removing path choice, not by shortening paths.** I expected the benefit to be about hop count. It is not. When same-rank GPUs share a rail switch, there is exactly one route between them, so ECMP has nothing to hash and two flows physically cannot be assigned to the same link by accident. 
...
**Tail latency is the only number that matters for a collective.** A collective finishes when its *slowest* member finishes, so 127 fast flows and one straggler is exactly as bad as 128 slow ones.

**There are two distinct regimes.** Below ~10 KB, completion time is round-trip propagation and hop count decides. Above ~100 KB, it is bytes divided by bandwidth and contention decides. 
